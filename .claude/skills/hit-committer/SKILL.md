...
name: hit-committer
description: >-
  Craft atomic Git commits in the Hit format — `[type] Description` titles (lowercase type, capitalized intent, ≤72 chars) with module-level body bullets. Use when committing in a Hit repo: "commit", "commit this", "make atomic commits", or when git-github-flow hands off its commit step. Commits only — no branches, push, or PRs (that's git-github-flow); refuses to commit onto protected `main` / `release/*`.
---
